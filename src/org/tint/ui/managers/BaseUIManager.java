...
import org.cybrosys.customisation.FrameView;
...
public abstract class BaseUIManager implements UIManager  {//, WebViewFragmentListener {
	...
	public FrameView chrometabView ;
	...
	public BaseUIManager(TintBrowserActivity activity) {
		...
		startHandler();
		chrometabView = (FrameView)getMainActivity().findViewById(R.id.chromeTabHost);
	...
	public void addTab(boolean loadHomePage, boolean privateBrowsing) {
		if (loadHomePage) {
			addTab(
					...
					false,
					privateBrowsing , false , 0 , 0);
		} else {
			addTab(null, false, privateBrowsing , false , 0 , 0);
		}
	}
					...
					if (!isCurrentTabReusable()) {
						addTab(url,
								...
								PreferenceManager.getDefaultSharedPreferences(this.getMainActivity()).
									getBoolean(Constants.PREFERENCE_INCOGNITO_BY_DEFAULT, false) , false , 0 , 0);
					} else {
						loadUrl(url);
					}					
				} else {
				...
				if (intent.hasExtra(Constants.EXTRA_ACTION_ID)) {
					...
					case TintBrowserActivity.CONTEXT_MENU_OPEN_IN_NEW_TAB:
						...
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT, -1)) {
							...
							addTab(intent.getStringExtra(Constants.EXTRA_URL), false, intent.getBooleanExtra(Constants.EXTRA_INCOGNITO, false) , false , 0 , 0);
						}
						break;
					...
					case TintBrowserActivity.CONTEXT_MENU_OPEN_IN_BACKGROUND:
						...
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT, -1)) {
							...
							addTab(intent.getStringExtra(Constants.EXTRA_URL), true, intent.getBooleanExtra(Constants.EXTRA_INCOGNITO, false) , false , 0 , 0);
						}
						break;
	...
	public void onReceivedIcon(WebView view, Bitmap icon) {
		...
		MoveController instance = (MoveController)getMainActivity().getApplicationContext();
		Iterator<Map.Entry<UUID, TabletWebViewFragment>> it = instance.tabletUIManagerinstance.mFragmentsMap.entrySet().iterator();
		while (it.hasNext()) {
		  Entry<UUID, TabletWebViewFragment> entry = it.next();
		
		  // Remove entry if key is null or equals 0.
		  if (entry.getKey() != null ) {
		    //Log.e(entry.getKey() + " key" , "  " + getWebViewByTabId(entry.getKey()));
		    if ( getWebViewByTabId(entry.getKey()) == view)
		    	chrometabView.setFavIconByUUID(entry.getKey(), icon);
		  }
		}
	}
	...
	public boolean isFullScreen() {		
		if ( PreferenceManager.getDefaultSharedPreferences(mActivity).getBoolean(Constants.PREFERENCE_IS_TAB, false))
			return true;
		else
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case FOCUS_NODE_HREF:
					...
					case TintBrowserActivity.CONTEXT_MENU_OPEN_IN_NEW_TAB:
						addTab(url, false, msg.arg2 > 0 ? true : false , false , 0 , 0);
						break;
						
					case TintBrowserActivity.CONTEXT_MENU_OPEN_IN_BACKGROUND:
						addTab(url, true, msg.arg2 > 0 ? true : false , false , 0 , 0);
						break;
